                        ...
                        <div class="col-12">
                            ...
                            <div class="card">
                                <div class="card-header">
                                    <h3 class="card-title">Edit Data Mentor</h3>
                                </div>
                                <!-- /.card-header -->
                                <div class="card-body">
                                    <form action="{{ route('MentorUpdate') }}" method="post"
                                        enctype="multipart/form-data">
                                        @csrf
                                        <div class="card-body">
                                            @foreach ($editMentor as $em)
                                            <div class="form-group">
                                                <label for="foto">Foto</label>
                                                <input type="hidden" class="form-control" id="id" name="id" value="{{ $em->id }}">
                                                <input type="file" class="form-control" id="foto" name="foto" value="{{ $em->img }}">
                                            </div>
                                            <div class="form-group">
                                                <label for="nama">Nama</label>
                                                <input type="text" class="form-control" id="nama" placeholder="Masukkan nama mentor" name="nama" value="{{ $em->nama }}">
                                            </div>
                                            <div class="form-group">
                                                <label for="asal">Asal</label>
                                                <input type="text" class="form-control" id="asal" placeholder="Masukkan asal mentor" name="asal" value="{{ $em->asal }}">
                                            </div>
                                            <div class="form-group">
                                                ...
                                                <select class="form-control select2" style="width: 100%;" name="tutor_id">
                                                    ...
                                                    @if ($ttr->id == $em->tutor_id)
                                                        <option value="{{ $em->tutor_id }}" selected>{{ $ttr->name }}
                                                        </option>
                                                    @else
                                                        <option value="{{ $em->tutor_id }}">{{ $ttr->name }}</option>
                                                    @endif
                                                    @endforeach
                                                </select>
                                            </div>
